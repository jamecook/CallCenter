...
namespace ClientPhoneWebApi.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public IEnumerable<Dto.Product> Get()
        {
            ...
            Logger.Log(LogLevel.Warning,"tetete");
            var connectionString = string.Format("server={0};uid={1};pwd={2};database={3};charset=utf8", "192.168.1.130",
                "asterisk", "mysqlasterisk", "CallCenter");
            using (var conn = new MySqlConnection(connectionString))
            {
                conn.Open();
                using (var cmd = new MySqlCommand($"select 123 as UserId", conn)
                )
                {
                    using (var dataReader = cmd.ExecuteReader())
                    {
                        if (dataReader.Read())
                        {
                                Logger.Log(LogLevel.Warning,$"{dataReader.GetInt32("UserId")}"); 
                        }

...
namespace RequestServiceImpl
{
 ...
 //       }
        public string GetActiveCallUniqueIdByCallId(string callId)
        {
            ...
            if (string.IsNullOrEmpty(callId))
                return null;
            var query = $@"SELECT case when (A.UniqueID <= ifnull(A2.UniqueID,A.UniqueID)) then A.UniqueID else A2.UniqueID end uniqueId FROM asterisk.ActiveChannels A
 left join asterisk.ActiveChannels A2 on A2.BridgeId = A.BridgeId and A2.UniqueID <> A.UniqueID
 where A.call_id like '{callId}%'";
            ...
            if(!string.IsNullOrEmpty(retVal))
                    return retVal;
                query = $@"SELECT case when (A.UniqueID <= ifnull(A2.UniqueID,A.UniqueID)) then A.UniqueID else A2.UniqueID end uniqueId FROM asterisk.ChannelHistory A
...
timestampdiff(SECOND, C.CreateTime, ifnull(C.BridgedTime, C.EndTime)) AS WaitingTime,
...
EndTime,BridgedTime,MonitorFile,TalkTime,WaitingTime,ClearWaitingTime,u.id AS userId,
                ...
                using (var dataReader = cmd.ExecuteReader())
                {
                    ...
                    while (dataReader.Read())
                    {
                        ...
                        callList.Add(new CallsListDto
                        {
                            ...
                            TalkTime = dataReader.GetNullableInt("TalkTime"),
                            WaitingTime = dataReader.GetNullableInt("WaitingTime"),
                            ClearWaitingTime = dataReader.GetNullableInt("ClearWaitingTime"),

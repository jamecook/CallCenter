...
namespace DevinoSender
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var sender = new DevinoSms.SmsServiceSoapClient();
            var sessionID = sender.GetSessionID(ConfigurationManager.AppSettings["DevinoUser"], ConfigurationManager.AppSettings["DevinoPassword"]);
            //Проверка баланса
            ...
            if (Convert.ToBoolean(ConfigurationManager.AppSettings["EnableSend"])==false)
            {
                ...
                {
                    using (var cmd = new MySqlCommand(@"update CallCenter.SMSRequest set message_id = -1,state_id=0,sms_count=0 where id = @smsId;", dbConnection))
                    {
            ...
            foreach (var sms in getStateList)
            {
                ...
                using (var cmd = new MySqlCommand(@"update CallCenter.SMSRequest set state_id = @StateId,state_desc = @StateDesc,
    ...
    S.price, S.message_id, S.date_utc, S.create_date FROM CallCenter.SMSRequest S ";
            if (inWork)
                sql += "where message_id is not null and (state_id is null or (state_id < 0 or state_id > 10))";
            else
                sql += "where message_id is null";
            using (
